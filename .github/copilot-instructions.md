...
8. [Miner Adapter Specific Patterns](#miner-adapter-specific-patterns)
...
### Miner Adapter Specific Context

This adapter connects ioBroker to cryptocurrency mining hardware and pools through various miner APIs:

- **Primary Purpose**: Monitor and control crypto mining hardware (ASIC miners, GPU rigs)
- **Supported Miners**: Braiins OS (BOS+), Luxor, and other mining platforms
- **Key Features**:
  - Device management through ioBroker device manager
  - Real-time mining statistics (hashrate, temperature, power consumption)
  - Miner control capabilities (start/stop, configuration)
  - Multi-miner support with MinerFactory pattern
  - Encrypted device credentials storage
  - WebSocket and HTTP API communication

- **API Integration Patterns**:
  - Most miners use HTTP/WebSocket APIs for communication
  - Authentication varies by miner type (API keys, username/password)
  - Real-time data updates via polling or WebSocket connections
  - Device discovery may not always be possible (manual configuration required)

- **Testing Considerations**:
  - Mock miner responses for unit tests (miners not accessible during CI)
  - Provide example data files for different miner types
  - Integration tests should use simulated miner data
  - Network connectivity to actual miners cannot be assumed
...
## Miner Adapter Specific Patterns

### Miner Factory Pattern
The adapter uses a factory pattern for creating different miner instances:
```typescript
import { createMiner } from './lib/miner/miner/MinerFactory';
const miner = createMiner(minerType, settings);
```

### Device Management Integration
- Uses `@iobroker/dm-utils` for device manager integration
- Device credentials are encrypted/decrypted using `encryptDeviceSettings` and `decryptDeviceSettings`
- All devices managed through `MinerAdapterDeviceManagement` class

### State Management
- Mining statistics organized by categories (hashrate, temperature, power, etc.)
- Control states under `control.*` namespace
- Connection status in `info.connection` state

### API Communication
- HTTP requests use utility functions from `lib/utils/http-utils.ts`
- WebSocket connections handled per miner type
- Always implement proper error handling for network failures
- Use appropriate timeouts for API requests (miners may be slow to respond)

### Logging Standards
- Use structured logging with Logger class from `lib/miner/model/Logger.ts`
- Include miner identifier in log messages for multi-device setups
- Use appropriate log levels (Level enum)

### Resource Cleanup
- Always close miner connections in `onUnload()`
- Use `minerManager.closeAll()` to properly disconnect all miners
- Clean up timers and intervals for polling
